...
app.config['MYSQL_PASSWORD'] = 'panchkula'
app.config['MYSQL_DB'] = 'voyage'
...
def contactus():
    if request.method == 'POST':
        ...
        query = "INSERT INTO messages VALUES('{fname}','{mail}','{msg}')".format(
            fname=first_name, mail=email, msg=message)
        cur.execute(query)
...
def login():
    ...
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            session['stock1'] = account['stock1']
            # print(session['stock1'])
            msg = 'Logged in successfully !'
            # return render_template('index.html')
            # helper(session['stock1'])
            ...
            def get_historical(quote):
                ...
                df.to_csv(''+quote+'.csv')
                if(df.empty):
                    ts = TimeSeries(key='N6A6QT6IBFJOPJ70',
                                    output_format='pandas')
                    data, meta_data = ts.get_daily_adjusted(
                        symbol='BSE:'+quote, outputsize='full')
            # Format df
            ...
            def ARIMA_ALGO(df):
                ...
                for company in uniqueVals[:10]:
                    ...
                    train, test = quantity[0:size], quantity[size:len(
                        quantity)]
                    #fit in model
            ...
            def LSTM_ALGO(df):
                ...
                X_forecast = np.reshape(
                    X_forecast, (1, X_forecast.shape[0], 1))
                ...
                print(
                    "##############################################################################")
                print("Tomorrow's ", quote,
                      " Closing Price Prediction by LSTM: ", lstm_pred)
            ...
            def LIN_REG_ALGO(df):
                ...
                X_to_be_forecasted = np.array(
                    df_new.iloc[-forecast_out:, 0:-1])
            ...
            try:
                ...
                return render_template('result.html', quote=quote, arima_pred=round(arima_pred, 2), lstm_pred=round(lstm_pred, 2),
                                       ...
                                       etr=str(
                                           quote_ratios['enterpriseToRevenue']),
    ...
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            cursor.execute('INSERT INTO accounts VALUES (NULL, % s, % s, % s, %s, %s, %s, %s)',
                           (email, password, mobile, address, stock1, stock2, stock3))
            mysql.connection.commit()
